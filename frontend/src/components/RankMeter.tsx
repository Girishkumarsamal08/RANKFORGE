...
export default function RankMeter({ rank }: RankMeterProps) {
  if (rank === null) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 p-8 text-center text-zinc-500">
        <AlertCircle className="h-8 w-8 text-zinc-400 mb-2" />
        <h4 className="font-semibold text-zinc-700">No Rank Prediction Yet</h4>
        <p className="text-xs text-zinc-400 mt-1">Complete a mock test to calculate your rank.</p>
      </div>
    );
  }
    ...
    <div className={`rounded-2xl border p-6 ${stats.bg} transition-all`}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <span className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Predicted Rank Range</span>
          ...
          <p className="text-xs text-zinc-600 mt-1 leading-relaxed">{stats.tip}</p>
        </div>
      </div>
      ...
      <div className="mt-6">
        ...
        <div className="relative mt-1.5 h-2 w-full rounded-full bg-zinc-200/80 overflow-hidden">
          {/* Highlight ranges */}
          <div
            className="absolute top-0 left-0 h-full bg-gradient-to-r from-blue-500 via-emerald-400 via-amber-400 to-zinc-400"
            style={{ width: '100%' }}
          />
          {/* Rank pointer representation */}
          <div 
            className="absolute top-0 h-full w-1.5 bg-zinc-950 border-x border-white shadow-md"
            style={{ 
