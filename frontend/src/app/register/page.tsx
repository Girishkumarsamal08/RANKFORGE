  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.errors && Array.isArray(data.errors) && data.errors.length > 0) {
        // Zod validation errors — show the first field-level message
      ...
      <main className="flex flex-1 items-center justify-center p-6">
        <div className="w-full max-w-md rounded-2xl border border-zinc-800/80 bg-zinc-900/45 p-8 shadow-2xl shadow-black/40 backdrop-blur-lg">
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">FullName</label>
              <input
                type="text"
                value={name}
